...
const ApiController = {
  ...
  async getExercises(categoryId = null, offset = 0, limit = 20) {
    ...
    const exercises = data.results
      .map(ex => {
        // wger API returns language as either an integer (2) or a URL ending in "/2/"
        const eng = ex.translations.find(t =>
          t.language === this.LANG ||
          String(t.language).endsWith(`/${this.LANG}/`) ||
          String(t.language) === String(this.LANG)
        );
        // Fall back to first available translation if no English found
        const trans = eng || ex.translations[0];
        if (!trans || !trans.name || !trans.name.trim()) return null; // Skip if no name

        // Strip HTML tags from description
        const rawDesc = (trans.description || '').replace(/<[^>]+>/g, '').trim();
